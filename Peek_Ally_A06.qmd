...
<https://github.com/allydoris/Peek_Ally_201_A06>
:::

::: {.callout-tip icon="false"}
## Screenshot/image of RStudio

![Picture of window with Git](Peek_Ally_git.PNG)
:::
...
```{r}
Lebron_James_pts <- player_info[[2]][[4]]
Lebron_James_pts

Lebron_James_pts2 <- player_info$LJ$points
Lebron_James_pts2
```
...
```{r}
points_per_game <- lapply(player_info, function(player) {player$points/player$games_played})
points_per_game
```
...
```{r}
point_per_game_vec <- sapply(points_per_game, function(x) round(x, 2))
print(point_per_game_vec)
```
...
mat_rand2 <- apply(mat_rand, MARGIN = 1:2, 
  function(x){
    round(x, -(nchar(x)-1))
  }
)
...
```{r}
co_med <- apply(CO_max_temps[, c('jan', 'feb', 'mar', 'apr', 'may', 'jun', 'jul', 'aug', 'sep', 'oct', 'nov', 'dec')], 1, median)
names(co_med) = 1895:2020 # reassign year labels to max temps

print(paste0('The maximum median annual temperature of ', max(co_med), ' occured in ', names(which.max(co_med)), ' and the minimum median annual temperature of ', min(co_med), ' occured in ', names(which.min(co_med)), '.'))
...
sum_digit <- function(num){
  if(num %/% 10 == 0){
    ...
    new_num = num %/% 10
    return(num %% 10 + sum_digit(new_num))
  }
}
